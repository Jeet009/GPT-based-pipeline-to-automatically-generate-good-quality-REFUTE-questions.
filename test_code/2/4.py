# Given a positive integer, return the sum of all odd numbers from 1 up to this number.
...
def run_test_sum_odd_numbers(func):
    """
    Run test cases for the given function to calculate the sum of odd numbers up to n
    ...
    class TestSumOddNumbers(unittest.TestCase):
        ...
        def test_hidden_cases(self):
            ...
            expected = [sum_odd_numbers(x) for x in inputs]
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(TestSumOddNumbers)
    # run Test Suite
...
if __name__ == "__main__":
    successful_inputs, failed_inputs, _, __ = run_test_sum_odd_numbers(
        sum_odd_numbers)
